...
+'---原文开始---\n'+txt+'\n---原文结束---';
    ...
    function _refreshRLSel(){if(!_popup)return;var rd=_loadRL();var $s=_popup.find('#mp-rlsel');$s.empty().append('<option value="">— 选择预设 —</option>');rd.list.forEach(function(p){$s.append('<option value="'+_e(p.name)+'"'+(p.name===rd.active?' selected':'')+'>'+_e(p.name)+'</option>');});}+'【禁词列表】替换为近义表达：\n{{WORDS}}\n\n'
+'【限制词列表】超出次数的替换：\n{{LIMITS}}\n\n'
+'【禁用句式】改写为其他表达：\n{{PATTERNS}}';
...
+'<div class="mp-hdr"><div class="mp-t1">🔍 AI输出审校器</div><div class="mp-t2">禁词替换 · 限制用词 · 润色改写</div><div class="mp-cr">制作：是猫猫呦|仅发精神病院| v'+_ver+'</div></div>'
...
+'</div>';
    ...
    function _addMenu(){if(!_jq)return;var doc=(window.parent||window).document;var menu=_jq('#extensionsMenu',doc);if(!menu.length){setTimeout(_addMenu,2000);return;}if(_jq('#'+_id+'-mi',menu).length)return;var wrap=_jq('<div class="extension_container interactable" id="'+_id+'-mi" tabindex="0"></div>');var item=_jq('<div class="list-group-item flex-container flexGap5interactable" title="猫猫审校器"><div class="fa-fw fa-solid fa-paw extensionsMenuExtensionButton"></div><span>猫猫审校器</span></div>');item.on('click',async function(ev){ev.stopPropagation();var btn=_jq('#extensionsMenuButton',doc);if(btn.length&&menu.is(':visible')){btn.trigger('click');await new Promise(function(r){setTimeout(r,120);});}await _openUI();});wrap.append(item);menu.append(wrap);}
    ...
    var _att=0;function _init(){_att++;if(_checkReady()){_l();_addMenu();_listen();_initTheme();console.log('[猫猫审校]v'+_ver+'初始化完成');}else if(_att<30)setTimeout(_init,2000);else console.warn('[猫猫审校]初始化超时，ST可能未就绪');}
...
+'</div>';
    ...
    function _addMenu(){if(!_jq)return;var doc=(window.parent||window).document;var menu=_jq('#extensionsMenu',doc);if(!menu.length){setTimeout(_addMenu,2000);return;}if(_jq('#'+_id+'-mi',menu).length)return;var wrap=_jq('<div class="extension_container interactable" id="'+_id+'-mi" tabindex="0"></div>');var item=_jq('<div class="list-group-item flex-container flexGap5 interactable" title="猫猫审校器"><div class="fa-fw fa-solid fa-paw extensionsMenuExtensionButton"></div><span>猫猫审校器</span></div>');item.on('click',async function(ev){ev.stopPropagation();var btn=_jq('#extensionsMenuButton',doc);if(btn.length&&menu.is(':visible')){btn.trigger('click');await new Promise(function(r){setTimeout(r,120);});}await _openUI();});wrap.append(item);menu.append(wrap);}
